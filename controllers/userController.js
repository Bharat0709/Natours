...
exports.resizeUserPhoto = catchAsync(async (req, res, next) => {
  // Check if req.file exists after setting the filename
  if (!req.file) {
    return next();
  }
  ...
  sharp(req.file.buffer)
    ...
    .toFile(`public/img/users/${req.file.filename}`, (err, info) => {
      if (err) {
        // Handle any errors from the sharp operation
...
exports.updateMe = catchAsync(async (req, res, next) => {
  // Usage in your updateMe middleware
...
exports.createUser = (req, res) => {
  res.status(500).json({
    status: 'error',
    message: 'This function is not defined yet!! Please Use Sign Up ',
  });
};
